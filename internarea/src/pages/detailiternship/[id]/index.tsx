...
const index = () => {
  ...
  useEffect(() => {
    ...
    const fetchdata = async () => {
      ...
      try {
        const res = await axios.get(buildBackendApiUrl(`internship/${id}`));
  ...
  if (errorMessage || !internshipData) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="w-full max-w-md rounded-lg bg-white p-6 text-center shadow-lg sm:p-8">
          ...
          <p className="mt-3 text-gray-600">
            This internship is not available right now.
          </p>
          <Link
            href="/internship"
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-lg">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-green-500 text-sm">
              Posted on {formatPostedDate(internshipData.createdAt)}
            </span>
          </div>
        </div>
